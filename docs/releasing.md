...
The `Release` workflow runs for `v*` tags. It checks out the tag, installs dependencies, runs verification, builds the workspace, publishes unpublished package versions to npm, and creates a GitHub release for the tag.
